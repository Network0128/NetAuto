...
device = driver('10.1.1.11', 'ccnp', 'cisco')  # 장비 접속 객체 생성 (IP, 계정 정보 설정 - 아직 연결 안 됨)
device.open()                             # SSH 세션을 열고 실제 장비에 로그인 수행

output = device.get_facts()               # 장비의 고유 정보(버전, 시리얼, 호스트명 등)를 딕셔너리로 수집
print(json.dumps(output, indent=4))    # 수집된 딕셔너리를 보기 좋게 JSON 포맷(4칸 들여쓰기)으로 출력

device.close()                            # 작업 완료 후 세션 종료 (VTY 라인 점유 해제 및 리소스 반환)
...
device = driver('10.1.1.11', 'ccnp', 'cisco')  # 장비 연결 객체 생성 (IP, ID, PW 정의 - 실제 연결은 아직 안 됨)
device.open()                         # 실제 장비에 SSH 접속 시도 (로그인 수행)

output = device.get_facts()           # 장비의 기본 정보 수집 (호스트네임, 모델명, OS 버전, 시리얼 등)
print(json.dumps(output, indent=4))    # 파이썬 딕셔너리를 JSON 포맷으로 변환하여 들여쓰기(4칸) 적용 후 출력
print()                                # 결과 구분을 위한 빈 줄 출력

output = device.get_interfaces()      # 모든 인터페이스의 상세 정보 수집 (Up/Down 상태, MAC 주소, 속도 등)
print(json.dumps(output, indent=4))    # 수집 결과 JSON 형식으로 출력
print()                                # 빈 줄 출력

output = device.get_interfaces_counters() # 인터페이스별 트래픽 통계 수집 (전송/수신 패킷 수, 에러, 드랍 등)
print(json.dumps(output, indent=4))    # 수집 결과 JSON 형식으로 출력

device.close()                        # SSH 세션 종료 (중요: 작업이 끝나면 반드시 연결을 끊어 리소스 반환)
...
device = driver('10.1.1.11', 'ccnp', 'cisco')  # 장비 접속 정보 설정 (객체 생성)
device.open()                                  # 장비에 실제 SSH 연결 시도

output = device.get_mac_address_table()        # 장비의 MAC 주소 테이블(L2 정보) 전체 수집
print(json.dumps(output, indent=4))             # 결과를 들여쓰기된 JSON 형태로 출력

device.close()                                 # 작업 완료 후 세션 연결 종료
