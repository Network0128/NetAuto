...
device = driver('10.1.1.11', 'ccnp', 'cisco')  # 장비 연결 객체 생성 (IP, ID, PW 설정)
device.open()                         # SSH 세션을 열고 장비에 로그인
...
output = device.ping(destination_ip)

# 3. 결과 출력
# 설명: NAPALM의 ping 결과는 성공률, RTT(지연시간) 등을 포함한 딕셔너리 형태로 반환됩니다.
print(json.dumps(output, indent=4))

device.close()                        # 작업 완료 후 세션 종료 (필수)
...
output = device.ping(target_ip, count=1)
